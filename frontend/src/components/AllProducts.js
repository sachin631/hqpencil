...
import { getAllProductsData } from "../RTK/AllProductSlice";
...
const AllProducts = () => {
  ...
  if (getProduct && getProduct.length) {
    dispatch(getAllProductsData(getProduct));
  }
  console.log(getProduct);

  if(!getProduct||getProduct.length==0){
    return <h1 className="absolute text-[300px]">Loading---------</h1>
  }
        ...
        <div className="w-[90%] container mx-auto">
          <div className="flex gap-[67px] flex-wrap justify-center mb-10">
            {getProduct.map((curelem) => {
              return (
                <div
                  className="border-[1px] hover:shadow-lg p-3 rounded-3xl border-gray-300"
                  key={curelem._id}
                >
                  <div className="bg-gray-100 rounded-3xl p-3">
                    <NavLink to={`${curelem._id}`}>
                      {" "}
                      <img
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="cursor-pointer bg-red-100 rounded-3xl p-2 border-[1px] border-red-500">
                      <AiOutlineLike className="text-xl text-red-500" />
